...
public class JuryMemberHbRepository extends AbstractHbRepository<Long, JuryMember> implements JuryMemberRepository {
    ...
    public Optional<JuryMember> findByUsernamePassword(String username, String password) {
        ...
        try (Session session = sessionFactory.openSession()){
            ...
            JuryMember member = session.createQuery("from JuryMember where username=:username and password=:password", JuryMember.class)
                    .setParameter("username", username)
                    .setParameter("password", password)
                    .setMaxResults(1)
                    .uniqueResult();
            transaction.commit();
            result = member != null ? Optional.of(member) : result;
        } catch (Exception e){
    ...
    protected Query<JuryMember> getFindQuery(Session session, Long aLong) {
        return session.createQuery("from JuryMember where id=:id", JuryMember.class).setParameter("id", aLong);
    }
