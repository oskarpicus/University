...
abstract class AbstractHbRepository<ID extends Serializable, E extends Entity<ID>> implements Repository<ID,E> {
    ...
    private Optional<E> merge(E entity) {
        System.out.println("MERGE ABSTRACT HB REPOSITORY");
        logger.traceEntry("Entry Merge {}", entity);
        Transaction transaction = null;
        Optional<E> result = Optional.empty();
        try (Session session = sessionFactory.openSession()){
            transaction = session.beginTransaction();
        ...
        logger.traceExit("Exit Merge result {}", result);
        return result;
    }
