    ...
    public void add(Polynomial other) {
        ...
        while (iterator.hasNext()) {
            ...
            if (findResult == null) {
                ...
                findResult.getLock().lock();
                Monomial monomial = findResult.getElement();
                findResult.setElement(new Monomial(monomial.getCoefficient() + current.getElement().getCoefficient(), monomial.getExponential()));

                if (findResult.getElement().getCoefficient() == 0) {
                    Monomial element = findResult.getElement();
                    findResult.getLock().unlock();
                    this.monomials.delete(element);
                } else {
                    findResult.getLock().unlock();
                }
            }
