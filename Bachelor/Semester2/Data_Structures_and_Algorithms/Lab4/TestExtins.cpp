...
void test_nou() {
	...
	m.adauga(2, 2);
	m.adauga(3, 3);
	IteratorMD i = m.iterator();
	assert(i.element().first == 1 && i.element().second ==1); //este pozitionat pe primul element
	i.precedent();
	assert(!i.valid());
	try {
		i.precedent();
	...
	IteratorMD i2 = m.iterator();
	i2.urmator(); //suntem pozitionati acum pe al 2-lea element
	assert(i2.element().first == 2 && i2.element().second == 2);
	i2.precedent(); //mergem la primul element
	assert(i2.element().first == 1 && i2.element().second == 1);
	i2.urmator(); // mergem inapoi la al doilea element
	assert(i2.element().first == 2 && i2.element().second == 2);
}
...
void testAllExtins() {
	...
	testCreeaza();
	cout << "GATA\n";
	testAdauga(); cout << "GATA\n";
	testSterge(); cout << "GATA\n";
	testIterator(); cout << "GATA\n";
	testQuantity();
}